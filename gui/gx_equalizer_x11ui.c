...
static void resize_event(gx_equalizerUI *ui);
...
cairo_surface_t *cairo_image_surface_create_from_stream (gx_equalizerUI* ui, const unsigned char* name) {
...
static LV2UI_Handle instantiate(const struct _LV2UI_Descriptor * descriptor,
	...
	gx_equalizerUI* ui = (gx_equalizerUI*)malloc(sizeof(gx_equalizerUI));
	...
	ui->controls[0] = (gx_controller) {{1.0, 1.0, 0.0, 1.0, 1.0}, {570, 90, 91, 91}, false,"POWER", BSWITCH, BYPASS};
	ui->controls[1] = (gx_controller) {{0.5, 0.5, 0.0, 1.0, 0.01}, {80, 15, 91, 91}, false,"35-680HZ", KNOB, FREQ2}; //35-680HZ
	ui->controls[2] = (gx_controller) {{0.5, 0.5, 0.0, 1.0, 0.01}, {200, 15, 91, 91}, false,"150-3KHZ", KNOB, FREQ1}; //150-3KHZ
	ui->controls[3] = (gx_controller) {{0.5, 0.5, 0.0, 1.0, 0.01}, {320, 15, 91, 91}, false,"450-8.5KHZ", KNOB, FREQ3}; //450-8.5KHZ
	ui->controls[4] = (gx_controller) {{0.5, 0.5, 0.0, 1.0, 0.01}, {440, 15, 91, 91}, false,"750-15KHZ", KNOB, FREQ4}; //750-15KHZ
	ui->controls[5] = (gx_controller) {{0.5, 0.5, 0.0, 1.0, 0.01}, {80, 125, 91, 91}, false,"BOOST", KNOB, BOOST1};
	ui->controls[6] = (gx_controller) {{0.5, 0.5, 0.0, 1.0, 0.01}, {200, 125, 91, 91}, false,"BOOST", KNOB, BOOST2};
	ui->controls[7] = (gx_controller) {{0.5, 0.5, 0.0, 1.0, 0.01}, {320, 125, 91, 91}, false,"BOOST", KNOB, BOOST3};
	ui->controls[8] = (gx_controller) {{0.5, 0.5, 0.0, 1.0, 0.01}, {440, 125, 91, 91}, false,"BOOST", KNOB, BOOST4};
	...
	ui->width = ui->init_width; // -140 + (60 * CONTROLS);
	...
	ui->frame = cairo_image_surface_create (CAIRO_FORMAT_ARGB32, 101, 121);
	...
	*widget = (void*)ui->win;
	...
	ui->rescale.xc = (double)ui->width/ui->init_width; //-140 + (60 * CONTROLS));
...
static void cleanup(LV2UI_Handle handle) {
	gx_equalizerUI* ui = (gx_equalizerUI*)handle;
	cairo_destroy(ui->cr);
...
static void draw_indicator_ring( cairo_t *cr, double knobstate, double ind_radius, 
	...
	double angle2 = scale_zero + 0.505 * 2 * (M_PI - scale_zero);
	double angle3 = scale_zero + 0.495 * 2 * (M_PI - scale_zero);
	...
	cairo_stroke(cr);
	cairo_arc_negative (cr, x_center , y_center, ind_radius,
		  add_angle + angle3, add_angle  + 380 * (M_PI/180));
	cairo_stroke(cr);
	...
	if (knobstate>0.505) {
		...
		cairo_arc (cr, x_center, y_center, ind_radius,
			  add_angle + angle2, add_angle + angle);
		cairo_stroke(cr);
	} else if (knobstate<0.495) {
		cairo_set_source_rgb(cr,0.5, 0.05, 0.05);
		cairo_arc_negative (cr, x_center, y_center, ind_radius,
			  add_angle + angle3, add_angle + angle);
		cairo_stroke(cr);
	}
...
static void knob_expose(gx_equalizerUI *ui,gx_controller* knob) {
	...
	if (knob->port >=BOOST1 ) {
		...
		if (knobstate < 0.495) cairo_set_source_rgba (ui->crf, 0.8, 0.8, 0.8,0.8);
		cairo_text_extents(ui->crf,"Cut", &extents);
		cairo_move_to (ui->crf, knobx1-knob_x/3.5-extents.width/1.6, knoby1+knob_y/1.7+extents.height/1.4);
		cairo_show_text(ui->crf, "Cut");
		cairo_new_path (ui->crf);

		cairo_set_source_rgba (ui->crf, 0.6, 0.6, 0.6,0.6);
		if (knobstate > 0.505) cairo_set_source_rgba (ui->crf, 0.8, 0.8, 0.8,0.8);
		cairo_text_extents(ui->crf,"Boost", &extents);
		cairo_move_to (ui->crf, knobx1+knob_x/3.5-extents.width/2.3, knoby1+knob_y/1.7+extents.height/1.4);
		cairo_show_text(ui->crf, "Boost");
		cairo_new_path (ui->crf);
	} else if (knob->type == ENUM) {
	...
	if (knob->port <BOOST1 ) {
		...
		cairo_select_font_face (ui->crf, "Sans", CAIRO_FONT_SLANT_NORMAL,
								   CAIRO_FONT_WEIGHT_BOLD);
		cairo_text_extents(ui->crf,l , &extents);

		cairo_move_to (ui->crf, knobx1-extents.width/2, grow-6+extents.height);
		cairo_show_text(ui->crf, l);
		cairo_new_path (ui->crf);
	}
}

// draw the power switch (bypass)
static void bypass_expose(gx_equalizerUI *ui, gx_controller* switch_) {
	cairo_set_operator(ui->crf,CAIRO_OPERATOR_CLEAR);
	cairo_paint(ui->crf);
	cairo_set_operator(ui->crf,CAIRO_OPERATOR_OVER);
...
static void draw_controller(gx_equalizerUI *ui, gx_controller* controller) {
...
static void _expose(gx_equalizerUI *ui) {
...
static void controller_expose(gx_equalizerUI *ui, gx_controller * control) {
	cairo_push_group (ui->cr);
	cairo_scale (ui->cr, ui->rescale.x, ui->rescale.y);
...
static void send_controller_event(gx_equalizerUI *ui, int controller) {
...
static void check_value_changed(gx_equalizerUI *ui, int i, float* value) {
...
static void check_is_active(gx_equalizerUI *ui, int i, bool set) {
...
static bool aligned(int x, int y, gx_controller *control, gx_equalizerUI *ui) {
...
static bool get_active_ctl_num(gx_equalizerUI *ui, int *num) {
	bool ret = false;
	for (int i=0;i<CONTROLS;i++) {
...
static bool get_active_controller_num(gx_equalizerUI *ui, int *num) {
	for (int i=0;i<CONTROLS;i++) {
...
static void scroll_event(gx_equalizerUI *ui, int direction) {
	float value;
	int num;
...
static void enum_event(gx_equalizerUI *ui, int i) {
	float value;
...
static void motion_event(gx_equalizerUI *ui, double start_value, int m_y) {
...
static void set_key_value(gx_equalizerUI *ui, int set_value) {
	float value = 0.0;
	int num;
	if (get_active_controller_num(ui, &num)) {
...
static void key_event(gx_equalizerUI *ui, int direction) {
	float value;
	int num;
	if (get_active_controller_num(ui, &num)) {
		value = min(ui->controls[num].adj.max_value,max(ui->controls[num].adj.min_value, 
...
static void set_previous_controller_active(gx_equalizerUI *ui) {
	int num;
	if (get_active_controller_num(ui, &num)) {
...
static void set_next_controller_active(gx_equalizerUI *ui) {
	int num;
	if (get_active_controller_num(ui, &num)) {
		ui->controls[num].is_active = false;
		send_controller_event(ui, num);
...
static void get_last_active_controller(gx_equalizerUI *ui, bool set) {
	int num;
	if (get_active_controller_num(ui, &num)) {
...
static void event_handler(gx_equalizerUI *ui) {
	XEvent xev;
